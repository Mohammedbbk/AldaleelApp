...
const InfoBaseScreen = () => {
  ...
  const { contentKey, nationality, destination, tripData = null } = route.params || {};

  // Debug log parameters
  console.log("InfoBaseScreen params:", { contentKey, nationality, destination, hasTripData: !!tripData });
  ...
  React.useEffect(() => {
    ...
    async function fetchContent() {
      ...
      if (tripData) {
        switch (contentKey) {
          case 'visa': prefetchedData = tripData.visaRequirements; break;
          case 'local': prefetchedData = tripData.cultureInsights; break; // Use cultureInsights
          case 'currency': prefetchedData = tripData.currencyInfo; break;
          case 'health': prefetchedData = tripData.healthAndSafety; break;
          case 'transportation': prefetchedData = tripData.transportation; break;
          case 'language': prefetchedData = tripData.languageBasics; break;
        }
      }
      ...
      console.log(`[InfoBaseScreen] Pre-fetched ${contentKey} data not found or empty. Attempting API fetch.`);
      ...
      if (!nationality || !destination) {
        ...
        if (!cancelled) {
          setError(`Please provide your ${missingFields.join(" and ")} to view this information.`);
          setIsLoading(false);
        }
        return;
      }
      ...
      try {
        let fetchedContent;
        console.log(`[InfoBaseScreen] Fetching ${contentKey} for ${nationality} -> ${destination}`);
        ...
        if (!cancelled) {
          ...
          if (prefetchedData) {
            console.warn(`[InfoBaseScreen] API fetch failed for ${contentKey}, but using prefetched data as fallback.`);
            setDynamicContent(prefetchedData);
            setError(`Could not refresh ${contentKey} data, showing previously generated info. Error: ${e.message || 'Failed to fetch'}`); // Non-blocking error
          } else {
