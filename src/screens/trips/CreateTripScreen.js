...
export function CreateTripScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    const durationValid = duration && !isNaN(duration) && parseInt(duration, 10) > 0;
  ...
  }, [selectedPlace, selectedMonth, selectedYear, selectedTravelerStyle, selectedBudgetLevel, duration, nationality]);
  ...
  const debouncedSearchCities = useCallback(debounce(searchCities, 500), [OPENWEATHERMAP_API_KEY]);
  ...
  const handleNext = () => {
    if (!isFormValid) {
      Alert.alert("Incomplete Form", "Please fill in all fields, including duration and nationality.");
      return;
    }
  ...
  const renderSearchResults = () => {
    ...
    if (isLoading) {
      return <ActivityIndicator size="large" color="#00BFFF" className="mt-4" />;
    }

    if (searchResults.length === 0 && searchQuery.length >= 2) {
      return <Text className="text-center text-gray-500 mt-4">No cities found for "{searchQuery}".</Text>;
    }

    return (
      <ScrollView keyboardShouldPersistTaps="handled" className="max-h-60 mt-2 border border-gray-200 rounded-md">
        {searchResults.map((item, index) => (
          ...
          >
            <Text className="text-base text-gray-800">{item.name}, {item.state ? `${item.state}, ` : ''}{item.country}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
    );
      ...
      >
        ...
        <View className="flex-row items-center justify-between px-5 pt-2.5 pb-5 bg-white dark:bg-gray-900">
          ...
          <View className="flex-1 items-center">
            <Text className="text-2xl font-bold text-gray-900 dark:text-white">Plan Your Trip</Text>
          </View>
          {/* Step Indicator */}
          <Text className="text-base text-orange-500 font-semibold">Step 1/3</Text>
        </View>

        <ScrollView className="flex-1 px-5" keyboardShouldPersistTaps="handled">

          {/* --- Destination --- */}
          <Text className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-2 mt-5">Where to?</Text>
          <Text className="text-base text-gray-500 dark:text-gray-400 mb-4">Enter your desired city destination.</Text>
          <TouchableOpacity
          ...
          >
            ...
            <Text className={`flex-1 text-base ${destination ? 'text-gray-800 dark:text-gray-200' : 'text-gray-500 dark:text-gray-400'}`}>
              {destination || 'Search for a city...'}
            </Text>
          </TouchableOpacity>

          {/* --- Dates --- */}
          <Text className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-4">When are you going?</Text>
          <View className="flex-row justify-between items-center mb-6">
            {/* Month Selector */}
            <View className="flex-1 mr-2">
              <Text className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Month</Text>
              <View className="flex-row flex-wrap justify-between">
                {MONTHS.map((row, rowIndex) => (
                  <View key={`month-row-${rowIndex}`} className="flex-row justify-between w-full mb-1.5">
                    {row.map((month) => (
                      <TouchableOpacity
                        key={month}
                        className={`w-[31%] py-2.5 rounded-full items-center border-2 ${selectedMonth === month ? 'border-sky-500 bg-sky-50 dark:bg-sky-900' : 'border-gray-300 dark:border-gray-600'}`}
                        onPress={() => setSelectedMonth(month)}
                      >
                        <Text className={`font-semibold ${selectedMonth === month ? 'text-sky-600 dark:text-sky-300' : 'text-gray-700 dark:text-gray-400'}`}>{month}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                ))}
            ...
            {/* Year Selector */}
            <View className="w-24">
              <Text className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Year</Text>
              <TouchableOpacity
              ...
              >
                <Text className="text-base font-semibold text-gray-800 dark:text-gray-200">{selectedYear}</Text>
          ...
          <Text className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-2 mt-4">How long?</Text>
          <Text className="text-base text-gray-500 dark:text-gray-400 mb-3">Enter the total number of days for your trip.</Text>
          <View className="flex-row items-center bg-gray-100 dark:bg-gray-800 rounded-full p-4 mb-6 shadow shadow-black/5">
            <MaterialCommunityIcons name="calendar-range" size={22} color="#999" className="mr-3" />
            <TextInput
            ...
            />
            {duration ? <Text className="text-base text-gray-600 dark:text-gray-400 ml-2">days</Text> : null}
          </View>

          {/* --- ADDED: Nationality Input --- */}
          <Text className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-2 mt-4">Your Nationality?</Text>
          <Text className="text-base text-gray-500 dark:text-gray-400 mb-3">Needed for visa checks and personalized tips.</Text>
          <View className="flex-row items-center bg-gray-100 dark:bg-gray-800 rounded-full p-4 mb-6 shadow shadow-black/5">
          ...
          {/* --- Travel Style --- */}
          <Text className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-4 mt-4">Who's traveling?</Text>
          <View className="flex-row justify-between mb-6">
            {TRAVELER_STYLES.map((style) => (
              <TouchableOpacity
                key={style}
                className={`flex-1 mx-1 py-3 rounded-full items-center border-2 ${selectedTravelerStyle === style ? 'border-sky-500 bg-sky-50 dark:bg-sky-900' : 'border-gray-300 dark:border-gray-600'}`}
                onPress={() => setSelectedTravelerStyle(style)}
              >
                <Text className={`font-semibold ${selectedTravelerStyle === style ? 'text-sky-600 dark:text-sky-300' : 'text-gray-700 dark:text-gray-400'}`}>{style}</Text>
              </TouchableOpacity>
            ))}
          </View>

          {/* --- Budget --- */}
          <Text className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-4 mt-4">Budget level?</Text>
          <View className="flex-row justify-between mb-10">
            {BUDGET_LEVELS.map((level) => (
              <TouchableOpacity
                key={level}
                className={`flex-1 mx-1 py-3 rounded-full items-center border-2 ${selectedBudgetLevel === level ? 'border-sky-500 bg-sky-50 dark:bg-sky-900' : 'border-gray-300 dark:border-gray-600'}`}
                onPress={() => setSelectedBudgetLevel(level)}
              >
                <Text className={`font-semibold ${selectedBudgetLevel === level ? 'text-sky-600 dark:text-sky-300' : 'text-gray-700 dark:text-gray-400'}`}>{level}</Text>
              </TouchableOpacity>
            ))}
          </View>
      ...
      >
        ...
        <SafeAreaView className="flex-1 bg-white">
          ...
          <View className="flex-row items-center justify-between px-4 py-3 border-b border-gray-200">
            <TouchableOpacity onPress={() => setShowSearchModal(false)} className="p-1">
              <Ionicons name="close" size={28} color="#333" />
            </TouchableOpacity>
            <Text className="text-lg font-semibold text-gray-800">Search Destination</Text>
