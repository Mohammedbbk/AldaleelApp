...
import { SPECIAL_REQUIREMENTS, TRANSPORTATION_OPTIONS } from '../../config/constants'; // Ensure this path is correct
...
const styles = StyleSheet.create({
    ...
    errorText: { color: '#EF4444', fontSize: 14, textAlign: 'center', marginHorizontal: 20, marginBottom: 8 },
    keyboardAvoidingView: { flex: 1 },
    headerContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 10, paddingBottom: 20, backgroundColor: '#FFF' },
    ...
    sectionTitle: { fontSize: 24, fontWeight: 'bold', color: '#1F2937', marginBottom: 16, marginTop: 20 },
    optionsContainer: { marginBottom: 20 },
    optionTouchable: { flexDirection: 'row', alignItems: 'center', marginBottom: 14 },
    checkboxBase: { width: 22, height: 22, borderRadius: 4, borderWidth: 2, marginRight: 10 },
    ...
    textInput: { borderWidth: 1.5, borderColor: '#E5E7EB', borderRadius: 8, paddingHorizontal: 16, paddingVertical: 10, fontSize: 16, color: '#1F2937' },
    createButtonContainer: { marginBottom: 40, marginTop: 10 }, // Added marginTop
    createButton: { borderRadius: 999, paddingVertical: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
    createButtonEnabled: { backgroundColor: '#0EA5E9' },
    createButtonDisabled: { backgroundColor: '#9CA3AF' },
    createButtonContent: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center' },
    loadingContainer: { flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 10 },
    loadingText: { color: '#FFF', marginTop: 8, textAlign: 'center', fontSize: 14 },
    createButtonText: { color: '#FFF', fontSize: 18, fontWeight: '600' },
    iconStyle: { marginLeft: 8 },
    bottomButtonsContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 },
    startFreshButton: { borderWidth: 1.5, borderColor: '#0EA5E9', borderRadius: 999, paddingHorizontal: 20, paddingVertical: 12 },
    startFreshText: { color: '#0EA5E9', fontSize: 14, fontWeight: '600' },
    homeButton: { position: 'absolute', bottom: -5, right: 130, width: 50, height: 50, borderRadius: 25, backgroundColor: '#F3F4F6', justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.2, shadowRadius: 2, elevation: 2 },
});
...
export function TripDetailsScreen({ route, navigation }) {
    ...
    const handleCreateAdventure = async () => {
        ...
        console.log('Attempting to create trip with data:', JSON.stringify(tripData, null, 2)); // Log data being sent
        ...
        try {
            ...
            await createTrip(tripData, {
                ...
                onError: (err) => { // Use onError callback for service-level errors
            ...
            console.error('Synchronous error during createTrip call or in subsequent handling:', err);
            setLoading(false); // Ensure loading is stopped

            // Retry logic for specific network-related errors
            if (retryCount < 2 && (err.message.includes('timed out') || err.message.includes('Network request failed') || err.message.includes('Failed to fetch'))) {
                setRetryCount(prev => prev + 1);
                Alert.alert(
                    i18n.t('tripDetails.alerts.serverStarting.title'),
                    i18n.t('tripDetails.alerts.serverStarting.message'),
                    [
                        { text: i18n.t('tripDetails.alerts.serverStarting.cancel'), style: 'cancel', onPress: () => setLoading(false) }, // Ensure loading stops on cancel
                        { text: i18n.t('tripDetails.alerts.serverStarting.retry'), onPress: handleCreateAdventure } // No arrow function needed
                    ]
                );
            } else {
                ...
                Alert.alert(i18n.t('tripDetails.alerts.error.title'), err.message || i18n.t('tripDetails.alerts.error.message'));
            }
        }
    };
    ...
    const renderRequirementOption = (item) => {
        ...
        return (
            ...
            >
                <View
                    style={[styles.checkboxBase, isSelected ? styles.checkboxSelected : styles.checkboxUnselected]}
                />
            ...
            >
                ...
                <View style={styles.headerContainer}>
                    ...
                    <View style={styles.headerTitleContainer}>
                        <Text style={styles.headerTitle}>{i18n.t('tripDetails.title')}</Text>
                    </View>
                    {/* Adjusted to not rely on absolute positioning if header structure changes */}
                    <Text style={styles.stepIndicator}>{i18n.t('tripDetails.stepIndicator')}</Text>
                </View>

                <ScrollView style={styles.scrollView} scrollEnabled={!loading} contentContainerStyle={{ paddingBottom: 40 }}>
                    ...
                    <View style={styles.optionsContainer}>
                        ...
                        <View style={styles.textInputContainer}>
                            <TextInput
                                ...
                                accessibilityRole="textbox" // Correct role
                            />
                        </View>
                    </View>
                    ...
                    <View style={styles.createButtonContainer}>
                        <TouchableOpacity
                            style={[styles.createButton, loading ? styles.createButtonDisabled : styles.createButtonEnabled]}
